...
export class caseSummaryPage {
    ...
    async completeCaseSummaryPage(mode: 'headless' | 'headed', page: Page = this.page, tagName?: string): Promise<void> {
        if (mode === 'headless') {
    ...
    async editTabsOnSummaryPageAndVerifyDownloadLink(mode: 'headless' | 'headed', page: Page = this.page): Promise<void> {
        if (mode === 'headless') {
        ...
        await commonLocators.clickOnLinkByName(page, 'Summary');
        await commonLocators.verifyFilledTextArea(page, 'Text 4 Edited on Documents tab');
        await commonLocators.clickOnLinkByName(page, 'Go to the Documents screen');
        await commonLocators.verifyButtonIsVisibleByName(page, 'Skip to allocation notes');
        await commonLocators.verifyFilledTextArea(page, 'Text 4 Edited on Documents tab');
        await commonLocators.verifyLinkIsVisibleByName(page, 'doc.pdf');
        await commonLocators.clickOnLinkByName(page, 'Summary');
        await commonLocators.verifyFilledTextArea(page, 'Text 4 Edited on Documents tab');
