...
export class allocateToTheCasePage {
    constructor(private page: Page) { }

    async completeAllocateToTheCasePage(mode: 'headless' | 'headed', page: Page = this.page): Promise<void> {
        if (mode === 'headless') {
            await expect(page).toHaveScreenshot('actual-allocateToTheCasePage.png', {
                fullPage: true,
                threshold: 0.2,
            });
        } else {
            console.log('allocateToTheCasePage - Skipping screenshot assertion in headed mode');
        }
