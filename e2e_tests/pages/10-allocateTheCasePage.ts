...
export class allocateTheCasePage {
    ...
    async editNotesAndCompleteAllocateTheCasePage(mode: 'headless' | 'headed', page: Page = this.page): Promise<void> {
        if (mode === 'headless') {
            await expect(page).toHaveScreenshot('actual-allocateTheCasePage.png', {
                fullPage: true,
                threshold: 0.2
        ...
        await commonLocators.verifyButtonIsVisibleByName(this.page, 'Save my notes without editing');
        await commonLocators.verifyLinkIsVisibleByName(this.page, 'Go back to review allocation');
        await commonLocators.clickOnButtonByName(this.page, 'Edit my notes before allocating the case');
        await commonLocators.fillTextInTextArea(this.page, "EDITED-AUTO-TESTING");
        await commonLocators.verifyFilledTextArea(this.page, "EDITED-AUTO-TESTING");
        await commonLocators.clickOnButtonByName(this.page, 'Save and allocate case');  
    }

        async editNotesAndCompleteAllocateTheCaseWithSensitiveInfoPage(mode: 'headless' | 'headed', page: Page = this.page): Promise<void> {
        if (mode === 'headless') {
            await expect(page).toHaveScreenshot('actual-allocateTheCasePage.png', {
                fullPage: true,
                threshold: 0.2
        ...
        await commonLocators.verifyLinkIsVisibleByName(this.page, 'Go back to review allocation');
        await commonLocators.clickOnButtonByName(this.page, 'Edit my notes before allocating the case');
        await commonLocators.fillTextInTextArea(this.page, 'EDITED-SENSITIVE-INFO');
        await commonLocators.verifyFilledTextArea(this.page, 'EDITED-SENSITIVE-INFO');
        await commonLocators.selectCheckBoxByName(this.page, 'Yes, it contains sensitive');
        await commonLocators.clickOnButtonByName(this.page, 'Save and allocate case');
    }
}
